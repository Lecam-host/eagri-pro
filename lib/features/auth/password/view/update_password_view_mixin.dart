...
mixin UpdatePasswordViewMixin on State<UpdatePasswordView> {
  ...
  void _listener(LoginState state) {
    if (state is UpdatePasswordSuccess) {
      ...
      AppHelper.showErrorMessage(
          context: context,
          content: LocaleKeys.password_update_successful_text.tr());
  ...
  void _updateButtonOnPressed(
      {required LoginBloc loginBloc, required RegisterState registerState}) {
    if (registerState is ForgotPasswordCheckSuccess) {
      HttpResponseModel<dynamic> response = AppHelper.checkPassword(
      ...
      if (response.statusCode == 200) {
        ...
        AppHelper.showErrorMessage(
            context: context, content: LocaleKeys.enter_valid_password.tr());
      }
    }
  }
}
